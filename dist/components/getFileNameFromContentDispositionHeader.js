 ...
 */
export var getFileNameFromContentDispositionHeader = function (contentDispositionHeader) {
    var _a;
    if (typeof contentDispositionHeader === 'string') {
        ...
        var keyValuePairs = contentDispositionHeader
            .split(';')
            .filter(function (v) { return v.trim().length; })
            .map(function (v) {
            var _a = v.split('=').map(function (w) { return w.trim(); }), key = _a[0], value = _a[1];
            return { key: key, value: value };
        });
        // Use uppercase for case-insensitive substring comparisons because lowercase has issues.
        var filenameValue = (_a = keyValuePairs.find(function (v) { return v.key.toUpperCase().includes('FILENAME'); })) === null || _a === void 0 ? void 0 : _a.value;
